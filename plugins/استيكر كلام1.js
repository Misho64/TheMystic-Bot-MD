...
let text
    if (args.length >= 1) {
        ...
        text = m.quoted.text;
    } else throw "[❗️] قم بكتابة الامر بالشكل التالي\n*.qc اهلا*";
   if (!text) return m.reply('[❗️] قم بكتابة الامر بالشكل التالي\n*.qc اهلا*');
    const who = m.mentionedJid && m.mentionedJid[0] ? m.mentionedJid[0] : m.fromMe ? conn.user.jid : m.sender; 
    const mentionRegex = new RegExp(`@${who.split('@')[0].replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s*`, 'g');
    const mishi = text.replace(mentionRegex, '');
   if (mishi.length > 30) return m.reply('*[❗️]لا يمكن أن يزيد النص عن 30 حرفًا*');
